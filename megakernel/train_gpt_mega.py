...
# ===== MEGA-KERNEL 1: Fused RMSNorm + MLP (reordering trick) =====
# Key: (x/rms) @ W = (x @ W) / rms  [per-row scalar distributes over matmul]
# One pass over x: accumulate GEMM output AND per-row sum(x²) simultaneously.
# Replaces: mlp_norm(x)*scale → FusedMLP(up_w, down_w)   [3 launches → 1 launch]
# Memory savings: eliminates HBM write of normed_x (~75MB per GPU per layer)
# Compliance: pure compute optimization, zero model behavior change.
_FUSED_RMSNORM_MLP = bool(int(os.environ.get("FUSED_RMSNORM_MLP", "0")))  # enable on H100 after benchmark


@triton.jit
def rmsnorm_linear_lrelu2_kernel(
    a_desc,      # Input x [M, K] — raw, un-normalized
    b_desc,      # Weight w1 [N, K] — up-projection
    c_desc,      # Output pre-act [M, N] — normed linear output (stored for bwd)
    aux_desc,    # Output post-act [M, N] — leaky_relu²(normed) (input to down_proj)
    M, N, K,
    ...
    for tile_id in tl.range(start_pid, num_tiles, NUM_SMS, flatten=True):
        ...
        if not FORWARD:
            ...
            c1 = c1 * tl.where(pre1 > 0, 2.0 * pre1, 0.5 * pre1)
        if FORWARD:
            inv_rms = (scale / tl.sqrt(sum_sq / K + eps)).to(dtype)
            c0 = c0 * inv_rms[:, None]
            c1 = c1 * inv_rms[:, None]
        c_desc.store([offs_am_c, offs_bn_c], c0)
        c_desc.store([offs_am_c, offs_bn_c + BLOCK_SIZE_N // 2], c1)
...
def rmsnorm_linear_lrelu2(a, w1, scale, eps, aux=None):
    M, K = a.shape
    N, K2 = w1.shape
    assert K == K2
    c = torch.empty((M, N), device=a.device, dtype=a.dtype)
    ...
    num_stages = 4 if forward else 3
    a_desc = TensorDescriptor.from_tensor(a, [BLOCK_SIZE_M, BLOCK_SIZE_K])
    b_desc = TensorDescriptor.from_tensor(w1, [BLOCK_SIZE_N, BLOCK_SIZE_K])
    c_desc = TensorDescriptor.from_tensor(c, [BLOCK_SIZE_M, BLOCK_SIZE_N // 2])
    aux_desc = TensorDescriptor.from_tensor(aux, [BLOCK_SIZE_M, BLOCK_SIZE_N // 2])
    grid = lambda _meta: (
        min(num_sms, triton.cdiv(M, BLOCK_SIZE_M) * triton.cdiv(N, BLOCK_SIZE_N)),
    )
    rmsnorm_linear_lrelu2_kernel[grid](
...
class FusedRMSNormMLPFunction(torch.autograd.Function):
    ...
    Forward: Triton kernel (saves normed_x HBM write).
    Backward: PyTorch ops (correct, Phase 1).
    """
    @staticmethod
    def forward(ctx, x, up_w, down_w, scale, eps):
        x_flat = x.reshape(-1, x.shape[-1])
        pre, post = rmsnorm_linear_lrelu2(
            x_flat, up_w.to(x_flat.dtype), scale, eps)
        out = F.linear(post, down_w.to(post.dtype))
    ...
    def backward(ctx, grad_output):
        ...
        dw_down = grad_flat.float().T @ post.float()
        d_post = grad_flat.float() @ down_w.float()
        ...
        x_normed = (x_f32 / rms * scale)
        dw_up = d_pre.T @ x_normed
        d_x_normed = d_pre @ up_w.float()
        # Backward through RMSNorm
        K = x_flat.shape[-1]
        dot = (d_x_normed * x_normed).sum(-1, keepdim=True) / K
        inv_rms = scale / rms
        dx = (inv_rms * (d_x_normed - x_normed * dot)).to(x.dtype)
...
# ── Kernel 2: Fused RMSNorm + QKV Projections ────────────────────────────────
# Replaces: attn_norm(x)*scale → [Q, K, V] projections  [4 launches → 3 launches]
# Memory savings: eliminates HBM write of normed_x_attn (~75MB per GPU per layer)
# Compliance: pure compute optimization, zero model behavior change.
_FUSED_RMSNORM_QKV = bool(int(os.environ.get("FUSED_RMSNORM_QKV", "0")))  # enable on H100 after benchmark
...
def rmsnorm_qkv_kernel(
    ...
    COMPUTE_RMS: tl.constexpr,  # True for Q (computes+stores inv_rms), False for K/V (loads)
):
...
# ── Kernel 2b: Unified QKV (x read ONCE, all three projections in one kernel) ─
# Autotune result: 1.356x at M=73728 vs 1.224x for sequential above.
# Memory savings: reads x 1× instead of 3× → saves 150MB HBM per layer.
# Fixed config: BM=64,BK=64,BNq=128,BNk=64,BNv=64,w=8,s=2 (H100 validated).

@triton.jit
def unified_qkv_kernel(
        x_ptr, qw_ptr, kw_ptr, vw_ptr,
        qo_ptr, ko_ptr, vo_ptr, rms_ptr,
        M, K, N_q, N_k, N_v,
        scale, eps,
        ...
        om = pm * BLOCK_M + tl.arange(0, BLOCK_M); mm = om < M
        onq = pn * BLOCK_Nq + tl.arange(0, BLOCK_Nq); mq = onq < N_q
        onk = pn * BLOCK_Nk + tl.arange(0, BLOCK_Nk); mk = onk < N_k
        onv = pn * BLOCK_Nv + tl.arange(0, BLOCK_Nv); mv = onv < N_v
        qa = tl.zeros((BLOCK_M, BLOCK_Nq), dtype=tl.float32)
        ka = tl.zeros((BLOCK_M, BLOCK_Nk), dtype=tl.float32)
        va = tl.zeros((BLOCK_M, BLOCK_Nv), dtype=tl.float32)
        ss = tl.zeros((BLOCK_M,), dtype=tl.float32)
        for ki in range(0, K, BLOCK_K):
            ok = ki + tl.arange(0, BLOCK_K); mok = ok < K
            xb = tl.load(x_ptr + om[:, None] * K + ok[None, :],
                         mask=mm[:, None] & mok[None, :], other=0.0)
            xf = xb.to(tl.float32); ss += tl.sum(xf * xf, axis=1)
            qw = tl.load(qw_ptr + onq[:, None] * K + ok[None, :],
                         mask=mq[:, None] & mok[None, :], other=0.0)
            qa = tl.dot(xb, tl.trans(qw), qa)
            kw2 = tl.load(kw_ptr + onk[:, None] * K + ok[None, :],
                          mask=mk[:, None] & mok[None, :], other=0.0)
            ka = tl.dot(xb, tl.trans(kw2), ka)
            vw2 = tl.load(vw_ptr + onv[:, None] * K + ok[None, :],
                          mask=mv[:, None] & mok[None, :], other=0.0)
            va = tl.dot(xb, tl.trans(vw2), va)
        ir = scale / tl.sqrt(ss / K + eps)
        tl.store(rms_ptr + om, ir.to(tl.float32), mask=mm)
        ir_bf = ir.to(tl.bfloat16)
        tl.store(qo_ptr + om[:, None] * N_q + onq[None, :],
                 (qa * ir_bf[:, None]).to(tl.bfloat16), mask=mm[:, None] & mq[None, :])
        tl.store(ko_ptr + om[:, None] * N_k + onk[None, :],
                 (ka * ir_bf[:, None]).to(tl.bfloat16), mask=mm[:, None] & mk[None, :])
        tl.store(vo_ptr + om[:, None] * N_v + onv[None, :],
                 (va * ir_bf[:, None]).to(tl.bfloat16), mask=mm[:, None] & mv[None, :])


def _unified_rmsnorm_qkv(x, q_w, k_w, v_w, scale, eps):
    """Single kernel: x read once, Q/K/V computed in parallel. 1.356x at M=73728."""
    M, K = x.shape
    N_q, N_k, N_v = q_w.shape[0], k_w.shape[0], v_w.shape[0]
    ...
    BM, BK = 64, 64
    BNq, BNk, BNv = 128, 64, 64
    grid = (triton.cdiv(M, BM), max(triton.cdiv(N_q, BNq), triton.cdiv(N_k, BNk)))
    unified_qkv_kernel[grid](
        x, q_w, k_w, v_w, q, k, v, inv,
        M, K, N_q, N_k, N_v, scale, eps,
        BLOCK_M=BM, BLOCK_K=BK, BLOCK_Nq=BNq, BLOCK_Nk=BNk, BLOCK_Nv=BNv,
        num_warps=8, num_stages=2,
    )
    return q, k, v, inv


def _rmsnorm_proj(x, w, scale, eps, inv_rms_buf=None):
    M, K = x.shape
    N = w.shape[0]
    ...
    BM, BN, BK = 128, 256, 64  # H100 autotune: BM=128,BN=256,BK=64 → 1.224x at M=73728
...
def _fused_rmsnorm_qkv(x, q_w, k_w, v_w, scale, eps):
    """Unified kernel: x read once, all of Q/K/V computed simultaneously. 1.356x at M=73728."""
    if x.is_cuda:
        return _unified_rmsnorm_qkv(x, q_w, k_w, v_w, scale, eps)
    else:
        rms   = torch.sqrt((x.float() ** 2).mean(-1, keepdim=True) + eps)
        x_n   = (x.float() / rms * scale).to(x.dtype)
...
def _qkv_x_normed(x_2d, inv_rms):
    return (x_2d.float() * inv_rms[:, None]).to(x_2d.dtype)


def _qkv_rmsnorm_bwd(d_xn, x_2d, inv_rms):
    K = x_2d.shape[-1]
    x_n = x_2d.float() * inv_rms[:, None]
    d_f = d_xn.float()
    dot = (d_f * x_n).sum(dim=-1, keepdim=True) / K
    return (inv_rms[:, None] * (d_f - x_n * dot)).to(x_2d.dtype)
...
class FusedRMSNormQKVFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, q_w, k_w, v_w, scale, eps):
        ...
        q, k, v, inv_rms = _fused_rmsnorm_qkv(
            x_2d, q_w.to(x_2d.dtype), k_w.to(x_2d.dtype), v_w.to(x_2d.dtype), scale, eps
        )
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, q_w, k_w, v_w, out_w, cu_seqlens=None, max_seqlen=0, pre_qkv=None):
        bsz, seqlen, dim = x.shape
        if pre_qkv is not None:
            # pre_qkv = (q_raw[B,T,d], k[B,T,Hkv,D], v[B,T,Hkv,D]) from fused kernel
            q_raw, k, v = pre_qkv
            q = q_raw.reshape(bsz, seqlen, self.num_heads, self.head_dim)
        else:
            # q_raw kept around as a tap point for attn_out_gate_src='q'.
...
class Block(nn.Module):
    ...
    def forward(self, x, x0, q_w, k_w, v_w, out_w, up_w, down_w, cu_seqlens=None, max_seqlen=0):
        ...
        if _FUSED_RMSNORM_QKV and self.training:
            bsz, seqlen, dim = x_in.shape
            x_2d = x_in.reshape(-1, dim)
            q_flat, k_flat, v_flat = FusedRMSNormQKVApply(
                x_2d, q_w.to(x_2d.dtype), k_w.to(x_2d.dtype), v_w.to(x_2d.dtype),
                self.ln_scale_factor, 1e-6,
            )
            pre_qkv = (
                q_flat.reshape(bsz, seqlen, dim),
                k_flat.reshape(bsz, seqlen, self.attn.num_kv_heads, self.attn.head_dim),
                v_flat.reshape(bsz, seqlen, self.attn.num_kv_heads, self.attn.head_dim),
            )
            attn_out = self.attn(
                x_in, q_w, k_w, v_w, out_w,
        ...
        if _FUSED_RMSNORM_MLP and self.training:
            mlp_result = FusedRMSNormMLP(
                x_out, up_w.to(x_out.dtype), down_w.to(x_out.dtype),
                self.ln_scale_factor, 1e-6,
            )
        else:
